...
//impliment hash table
//site: get the function from http://www.cs.yale.edu/homes/aspnes/pinewiki/C(2f)HashTables.html?highlight=%28CategoryAlgorithmNotes%29
...
void openHashTable(){
	char magic[] = "magic hash";
	char buf[10];
	fd = open("hashTable", O_RDWR);
	if(fd != -1){
		pread(fd, buf, 10, 0);
	...
	close(fd);
	fd = open("hashTable", O_RDWR | O_CREAT | O_TRUNC);
	pwrite(fd, buf, 10, 0);
	return;
}

/*void writeHashTable(char* key, char* value){
	char buf[128];
	sem_wait(hashLock);
	int index = hash(value) % 8000;
	pread(fd, buf, 128, 10 + index*128);
	if()
}*/
int main(int argc, char* argv[]){
	...
	hashLock = (sem_t*)calloc(1, sizeof(sem_t));
